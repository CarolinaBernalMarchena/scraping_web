        ...
        if fecha_primera or fecha_ultima:
            ...
            try:
                ...
                if fecha_ultima_formatted and fecha_primera_formatted == None:
                    if query:
                        query = query & qp.parse(f"fecha_primera:[00010101 to {fecha_ultima_formatted}]")
                    else:
                        query = qp.parse(f"fecha_primera:[00010101 to {fecha_ultima_formatted}]")

                if fecha_primera_formatted and fecha_ultima_formatted:
                    if query:
                        query = query & qp.parse(f"fecha_primera:[{fecha_primera_formatted} to {fecha_ultima_formatted}]")
                    else:
                        query = qp.parse(f"fecha_primera:[{fecha_primera_formatted} to {fecha_ultima_formatted}]")
